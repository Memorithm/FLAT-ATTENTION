...
- **MAA-12a**: preregister and explore a batch-level survivor floor that may rescue only Boolean-qualified candidates using deterministic F2/Zhegalkin support, while keeping Max-Plus outside semantic rescue and never reusing the MAA-11b holdout for tuning. The preserved exploratory observation found no tested floor that matched the Boolean-only quality diagnostics while reducing score evaluations, so this specific support-count rescue does not advance to confirmation. See [protocol](MAA_SURVIVOR_FLOOR_EXPLORATORY_PREREGISTRATION.md) and [observation](MAA_SURVIVOR_FLOOR_EXPLORATORY_OBSERVATION.md).
- **MAA-12b**: not opened from the MAA-12a support-count hypothesis; any later confirmatory floor policy requires a materially new exploratory result plus new policy/tuning/holdout identities.
- **MAA-13a**: derive deterministic F2 signatures directly from numerical Q/K using a preregistered signed-hyperplane projector, bridge the exact packed bits into the authoritative M13B.2 signature/Hamming contract, measure angular ranking/admission fidelity across frozen widths and thresholds, and preserve positive-scale invariance as a required negative control. The preserved observation found useful angular structure: the 128-bit half-width arm selected 1,030/2,048 logical candidates while retaining 251/256 dense-top2 occurrences with no empty case. It also confirmed 64/64 positive-scale signature collisions, so sign bits alone cannot recover magnitude. See [protocol](MAA_QK_SIGNATURE_EXPLORATORY_PREREGISTRATION.md) and [observation](MAA_QK_SIGNATURE_EXPLORATORY_OBSERVATION.md).
- **MAA-13b**: preregistered norm-aware continuation: preserve the 128-bit MAA-13a angular signature baseline, add a compact precomputable key-norm predicate, and evaluate the exact Boolean rule `near OR (medium AND high_norm)` through both direct Boolean semantics and the exact Zhegalkin->F2 split on a fresh variable-norm dataset. The preserved observation selected 1,299/2,048 logical candidates while retaining 255/256 dense-top2 occurrences, versus 1,531/2,048 for relaxed Hamming at the same top2 count. However relaxed Hamming retained lower O/LSE error, proving that top2 retention alone is not an adequate softmax-quality proxy. See [protocol](MAA_NORM_AWARE_QK_EXPLORATORY_PREREGISTRATION.md) and [observation](MAA_NORM_AWARE_QK_EXPLORATORY_OBSERVATION.md).
- **MAA-13c**: preregister a deterministic f64 dense-reference retained-softmax-mass oracle on a fresh 256x32 variable-norm panel, verify the mathematical LSE identity and output-error mass bound against FLAT, and compare dense, strict/relaxed Hamming, norm-only, norm-aware MAA, density-matched random and a non-deployable exact-score mass-oracle upper bound. See [protocol](MAA_RETAINED_SOFTMAX_MASS_PREREGISTRATION.md).
- **MAA-13d**: freeze, execute and preserve a fresh confirmatory holdout for the exploratory mass-aware tiered-recall router. The preserved confirmatory result authorizes physical qualification of that exact candidate, not a general performance claim.
- **MAA-13e**: physically qualify the frozen tiered-recall candidate without retuning it; the first merged execution bridge materializes its predicate into the FA-V888-1 StructuralCandidateSet sparse path. Physical timing/traffic evidence remains separate from logical selection counts.
- **MAA-14a**: add a deterministic monotonic adaptive-repair transport over StructuralCandidateSet. The base route and ordered per-row widening tiers are frozen separately from the external repair trigger; repair may add candidates but cannot silently remove earlier survivors. See [bootstrap](MAA_ADAPTIVE_REPAIR_BOOTSTRAP.md).
- **MAA-14b..14d**: preregister deployable repair signals, compare matched host controls, and only then consider a portable WGPU adaptive candidate. Dense exact-score retained mass is allowed as a non-deployable evaluation oracle, never as a hidden runtime trigger.
