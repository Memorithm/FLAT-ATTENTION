...
- **MAA-13c**: next evidence slice: preregister exact dense-reference retained softmax probability mass (or an equivalent stable exact mass diagnostic) on a fresh dataset, then compare angular/norm routing and density-matched controls under retained-mass plus O/LSE evidence before considering a finer tiered policy.
