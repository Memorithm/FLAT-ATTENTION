...
- **MAA-13a**: derive deterministic F2 signatures directly from numerical Q/K using a preregistered signed-hyperplane projector, bridge the exact packed bits into the authoritative M13B.2 signature/Hamming contract, measure angular ranking/admission fidelity across frozen widths and thresholds, and preserve positive-scale invariance as a required negative control. See [protocol](MAA_QK_SIGNATURE_EXPLORATORY_PREREGISTRATION.md).
- **MAA-13b**: only if MAA-13a shows useful Q/K-derived structure, investigate F2/Zhegalkin predicates over signature mismatch features on a fresh exploratory dataset; no confirmatory promotion is implied.
