...
- **MAA-13b**: preregistered norm-aware continuation: preserve the 128-bit MAA-13a angular signature baseline, add a compact precomputable key-norm predicate, and evaluate the exact Boolean rule `near OR (medium AND high_norm)` through both direct Boolean semantics and the exact Zhegalkin->F2 split on a fresh variable-norm dataset. Compare against strict/relaxed Hamming, norm-only and density-matched random controls, with dense FLAT O/LSE diagnostics. See [protocol](MAA_NORM_AWARE_QK_EXPLORATORY_PREREGISTRATION.md).
