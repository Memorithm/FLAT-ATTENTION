...
- **MAA-13b**: motivated exploratory continuation: preserve the Q/K angular signature baseline while adding an independent compact key-norm feature, then test Boolean/F2/Zhegalkin interactions on a fresh variable-norm dataset. MAA-13a's sign-only bits remain the baseline; no confirmatory promotion is implied.
