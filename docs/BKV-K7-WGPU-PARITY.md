...
It also binds the exact source revision, lockfile WGPU runtime identity, adapter
name, backend, driver/driver-info and vendor/device IDs captured from the same
dispatch adapter. It retains the signature width, threshold, exact packed
query/key words, CPU candidate IDs and WGPU candidate IDs. Malformed readback fails closed on a length mismatch or non-binary flag. A valid
but different WGPU candidate set is retained as explicit negative evidence with
`exact_candidate_set_match=false`. `require_exact_match_for()` then rejects that
record before any CPU-vs-WGPU performance comparison unless the timing harness
also supplies exactly matching source/runtime/adapter provenance. Parity from a
software Vulkan adapter therefore cannot authorize timing from a different
physical GPU or driver.
