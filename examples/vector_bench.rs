...
#[cfg(feature = "wgpu")]
fn main() {
    let scalar = WgpuFlatAttention::with_subgroup_policy_and_vectorization(
        WgpuSubgroupPolicy::Disable,
        false,
    )
    .expect("scalar Q4 WGPU context");
    let vector = WgpuFlatAttention::with_subgroup_policy_and_vectorization(
        WgpuSubgroupPolicy::Disable,
        true,
    )
    .expect("vec4 Q4 WGPU context");
    ...
    let scalar_median = measure(&scalar, &q, &k, &v, shape, config, iterations);
    let vector_median = measure(&vector, &q, &k, &v, shape, config, iterations);

    println!("adapter={}", scalar.adapter_name());
    ...
    println!(
        "q4_vec4_median_ms={:.3}",
        vector_median.as_secs_f64() * 1_000.0
    );
    println!(
        "scalar_over_vec4_ratio={:.3}x",
        scalar_median.as_secs_f64() / vector_median.as_secs_f64()
    );
}

#[cfg(not(feature = "wgpu"))]
fn main() {
    eprintln!("run with: cargo run --release --features wgpu --example vector_bench");
}
