...
fn run_case(
    ...
    let q_gpu = input_buffer(device, queue, &q, "flat-m27-q");
    let k_gpu = input_buffer(device, queue, &k, "flat-m27-k");
    let v_gpu = input_buffer(device, queue, &v, "flat-m27-v");
    ...
    let execute = || {
        ...
        let mut encoder = device.create_command_encoder(&wgpu::CommandEncoderDescriptor {
            label: Some("flat-m27-resident-grouped-forward"),
        });
        pipeline
            .encode(
                device,
...
fn main() {
    ...
    let (device, queue) = pollster::block_on(adapter.request_device(
        &wgpu::DeviceDescriptor {
            label: Some("flat-m27-resident-grouped-forward-sweep"),
    ...
    println!("benchmark=m27_resident_grouped_forward_sweep");
        ...
        for &seq_len in &[32_usize, 128, 512] {
            for &head_dim in &[32_usize, 64, 128] {
                for &causal in &[false, true] {
                    ...
                    let (median_us, p95_us) = run_case(&device, &queue, &pipeline, shape, config);
                    let logical_query_tokens_per_s = seq_len as f64 * 1.0e6 / median_us;
                    println!(
                        "1,{q_heads},{kv_heads},{seq_len},{head_dim},{causal},{median_us:.3},{p95_us:.3},{logical_query_tokens_per_s:.3}"
                    );
                }
            }
        }
