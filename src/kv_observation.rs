...
impl PagedKvPageObservation {
    #[must_use]
    pub const fn logical_page(self) -> usize { self.logical_page }
    #[must_use]
    pub const fn physical_page(self) -> usize { self.physical_page }
    #[must_use]
    pub const fn logical_token_start(self) -> usize { self.logical_token_start }
    #[must_use]
    pub const fn logical_token_end(self) -> usize { self.logical_token_end }
    #[must_use]
...
///
/// Equality means the captured page-table metadata is equal. It does not prove
/// that the resident K/V bytes or speculative cache branch are the same.
...
impl PagedKvObservation {
    ...
    #[must_use]
    pub const fn schema_version(&self) -> u32 { self.schema_version }
    #[must_use]
    pub const fn config(&self) -> PagedKvConfig { self.config }
    #[must_use]
    pub const fn telemetry(&self) -> PagedKvTelemetry { self.telemetry }
    #[must_use]
    pub fn pages(&self) -> &[PagedKvPageObservation] { &self.pages }
}
...
impl KvResidencyEvent {
    #[must_use]
    pub const fn new(sequence: u64, physical_page: usize, generation: u64, kind: KvResidencyEventKind) -> Self {
        Self { sequence, physical_page, generation, kind }
    }
    #[must_use]
    ...
    pub const fn generation(self) -> u64 { self.generation }
    #[must_use]
    pub const fn kind(self) -> KvResidencyEventKind { self.kind }
}
...
mod tests {
    ...
    #[test]
    fn capture_preserves_geometry_spans_and_fragmentation() {
        let mut table = PagedKvTable::new(PagedKvConfig { page_size: 4, physical_pages: 3 }).unwrap();
        table.append(6).unwrap();
        let observation = PagedKvObservation::capture(&table).unwrap();
        assert_eq!(observation.schema_version(), 1);
        assert_eq!(observation.config(), PagedKvConfig { page_size: 4, physical_pages: 3 });
    ...
    #[test]
    fn empty_observation_still_preserves_page_geometry() {
        let table = PagedKvTable::new(PagedKvConfig { page_size: 8, physical_pages: 5 }).unwrap();
        let observation = PagedKvObservation::capture(&table).unwrap();
        assert_eq!(observation.config(), PagedKvConfig { page_size: 8, physical_pages: 5 });
        assert!(observation.pages().is_empty());
    }

    #[test]
    fn reused_physical_page_has_a_new_generation_after_reset() {
        let mut table = PagedKvTable::new(PagedKvConfig { page_size: 2, physical_pages: 2 }).unwrap();
