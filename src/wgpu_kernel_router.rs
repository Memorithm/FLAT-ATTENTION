//! Prepared WGPU kernel routing for live host-I/O realization changes.
//!
//! The router compiles every admitted candidate before it becomes selectable.
//! Applying a candidate therefore changes only the active prepared route; it
//! does not compile a pipeline, mutate attention semantics, or silently fall
//! back to another candidate. This is intentionally narrower than resident
//! model/KV switching: each prepared candidate currently owns its own WGPU
//! context, so resident buffers are not transferable between routes.

use core::fmt;
use std::collections::BTreeSet;

use crate::kernel_candidates::{CandidateId, CandidateLifecycle, KernelCandidate};
use crate::kernel_ir::AttentionProblem;
use crate::{
...
mod tests {
    use super::*;
    use crate::kernel_candidates::{generate_candidates, SelectionPolicy};
    use crate::kernel_ir::AttentionProblem;
    use crate::{RuntimeDeviceCapabilities, WgpuSubgroupPolicy};
