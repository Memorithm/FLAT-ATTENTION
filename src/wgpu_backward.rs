//! M18 correctness-first WGPU backward recomputation pipeline.
//!
//! The portable kernel avoids floating-point atomics by assigning every output
//! scalar gradient to exactly one invocation. Forward operands are supplied in
//! one caller-owned storage buffer using `Q | K | V | dO | O | LSE`; gradients
//! are written as `dQ | dK | dV` into a second caller-owned storage buffer.
//! This keeps the path within conservative WGPU storage-binding limits.
//!
//! The pipeline owns no queue or framework tensor storage and never submits,
//! polls, maps, or synchronizes. This generation qualifies correctness only;
//! its deliberately redundant recomputation is not a performance claim.

use core::fmt;

use crate::{AttentionShape, FlatAttentionConfig, FlatAttentionError, FLAT_BACKWARD_RECOMPUTE_WGSL};

const BACKWARD_WORKGROUP_SIZE: usize = 64;

/// Packed buffer geometry required by the M18 portable backward kernel.
...
impl BackwardRecomputeLayout {
    /// Element offset of Q in `Q | K | V | dO | O | LSE`.
...
/// Caller-owned buffers for one M18 backward dispatch.
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::IndexSpaceExceeded { elements } => {
                write!(f, "backward recomputation exceeds WGPU u32 index space at {elements} elements")
            }
...
/// Reusable portable M18 backward pipeline.
...
impl WgpuBackwardRecomputePipeline {
    ...
    pub fn layout(shape: AttentionShape) -> Result<BackwardRecomputeLayout, BackwardRecomputeError> {
        ...
        if packed_forward_elements > u32::MAX as usize {
            return Err(BackwardRecomputeError::IndexSpaceExceeded {
                elements: packed_forward_elements,
            });
        }
        if gradient_elements > u32::MAX as usize {
            return Err(BackwardRecomputeError::IndexSpaceExceeded {
                elements: gradient_elements,
            });
        }
    ...
    /// Record one recomputation backward dispatch into a caller-owned encoder.
    pub fn encode(
        &self,
        device: &wgpu::Device,
        ...
        let params_buffer = device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("flat-m18-backward-params"),
