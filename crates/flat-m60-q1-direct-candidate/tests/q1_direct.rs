...
use flat_m60_q1_direct_candidate::{Q1DirectError, Q1DirectMhaPipeline, Q1_DIRECT_VEC4_WGSL};
...
fn run_case(harness: &Harness, head_dim: usize, causal: bool) {
    ...
    let pipeline = Q1DirectMhaPipeline::new(&harness.device).unwrap();
    let output = pipeline
    ...
    let prepared = pipeline
        ...
        .unwrap();
    assert_eq!(prepared.dispatch_x(), shape.seq_len as u32);
...
fn shader_removes_non_reused_kv_workgroup_staging() {
    assert!(!Q1_DIRECT_VEC4_WGSL.contains("k_shared"));
    assert!(!Q1_DIRECT_VEC4_WGSL.contains("v_shared"));
    assert!(Q1_DIRECT_VEC4_WGSL.contains("let k_slice = k[global4]"));
    assert!(Q1_DIRECT_VEC4_WGSL.contains("v_slice = v[global4]"));
}

#[test]
fn contract_rejects_gqa_and_non_vec4_dimensions_without_device() {
    ...
    assert!(matches!(
        Q1DirectMhaPipeline::layout(gqa),
        Err(Q1DirectError::RequiresMha { .. })
    ));
    ...
    assert!(matches!(
        Q1DirectMhaPipeline::layout(d32),
        Err(Q1DirectError::UnsupportedHeadDim(32))
    ));
}
