// ADA-A1 isolated portable fused forward candidate — Q4 MHA.
//
// This file intentionally mirrors FLAT's qualified `shaders/flat_fwd.wgsl`
// geometry, bindings, staging, reductions, output layout, and dispatch. The
// only research mutation is the Online Softmax state update: the first
// admissible key initializes state with no exp, and every later admissible key
            ...
            loop {
                ...
                if (lane == 0u) {
                    if (participates) {
                        ...
                        if (running_sum_shared[qr] == 0.0) {
                            // First admissible key: (m, l, O) = (score, 1, V).
                            alpha = 0.0;
                            running_max_shared[qr] = score;
                            running_sum_shared[qr] = 1.0;
                        } else if (score <= previous_max) {
