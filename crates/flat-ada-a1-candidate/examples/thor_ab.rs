...
fn prepare_dispatch(
    ...
    let params_buffer = harness.device.create_buffer(&wgpu::BufferDescriptor {
        label: Some("flat-ada-a1-bench-params"),
...
fn read_f32(harness: &Harness, source: &wgpu::Buffer, len: usize) -> Vec<f32> {
    ...
    let staging = harness.device.create_buffer(&wgpu::BufferDescriptor {
        label: Some("flat-ada-a1-readback"),
        size: bytes,
...
impl GpuTimer {
    fn new(device: &wgpu::Device) -> Self {
        let query_set = device.create_query_set(&wgpu::QuerySetDescriptor {
            label: Some("flat-ada-a1-timestamps"),
            ty: wgpu::QueryType::Timestamp,
            count: 2,
        });
        let resolve = device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("flat-ada-a1-timestamp-resolve"),
            size: 16,
            usage: wgpu::BufferUsages::QUERY_RESOLVE | wgpu::BufferUsages::COPY_SRC,
            mapped_at_creation: false,
        });
        let readback = device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("flat-ada-a1-timestamp-readback"),
            size: 16,
            usage: wgpu::BufferUsages::COPY_DST | wgpu::BufferUsages::MAP_READ,
            mapped_at_creation: false,
        });
...
fn measure_gpu_ns(
    ...
    let mut encoder = harness
        .device
        .create_command_encoder(&wgpu::CommandEncoderDescriptor {
            label: Some("flat-ada-a1-gpu-timestamp"),
        });
    {
...
fn main() {
    ...
    .expect("ADA-A1 hardware benchmark requires a WGPU adapter");
    ...
    let (device, queue) = pollster::block_on(adapter.request_device(&wgpu::DeviceDescriptor {
        label: Some("flat-ada-a1-thor-ab"),
        required_features: wgpu::Features::TIMESTAMP_QUERY,
        required_limits: wgpu::Limits::downlevel_defaults(),
        ..Default::default()
    }))
    .expect("ADA-A1 timestamp-capable request_device failed");
    ...
    println!("benchmark=ada_a1_gpu_timestamp_ab");
    ...
    println!("sample_order=paired_interleaved_baseline_candidate_then_candidate_baseline");
    println!("uploads_in_timing=false");
    println!("readback_in_timing=false");
    println!("pipeline_compile_in_timing=false");
    println!("correctness_gate=cpu_oracle_and_qualified_q4_before_timing");
    println!("baseline=flat_fwd.wgsl");
    println!("candidate=flat_fwd_ada_a1.wgsl");
    println!("batch,heads,seq_len,head_dim,causal,baseline_median_ns,baseline_p95_ns,baseline_mad_ns,candidate_median_ns,candidate_p95_ns,candidate_mad_ns,speedup_baseline_over_candidate");
        ...
        for &head_dim in &head_dims {
            ...
            for causal in [false, true] {
                ...
                let candidate_actual = read_f32(
                    &harness,
                    &candidate_prepared.output,
                    candidate_prepared.combined_len,
                );
                assert_close(
                ...
                for _ in 0..warmup {
                    ...
                    execute_plain(&harness, &candidate, &candidate_prepared, inner_dispatches);
                }
                ...
                for sample in 0..samples {
                    if sample.is_multiple_of(2) {
                        ...
                        candidate_samples.push(measure_gpu_ns(
                            &harness,
                            &timer,
                            &candidate,
                            &candidate_prepared,
                ...
                let candidate_summary = summarize(candidate_samples);
                let speedup = baseline_summary.median_ns / candidate_summary.median_ns;
                println!(
                    "1,1,{seq_len},{head_dim},{causal},{:.3},{:.3},{:.3},{:.3},{:.3},{:.3},{speedup:.6}",
                    baseline_summary.median_ns,
                    baseline_summary.p95_ns,
                    baseline_summary.mad_ns,
                    candidate_summary.median_ns,
                    candidate_summary.p95_ns,
                    candidate_summary.mad_ns,
                );
            }
        }
    }
