...
use flat_ada_a1_candidate::ada_a1_branchless_wgsl;
use flat_attention::{
...
fn harness() -> Option<Harness> {
    ...
    let Some(adapter) = adapter else {
        if std::env::var_os("FLAT_REQUIRE_WGPU").is_some() {
            panic!("ADA-A1B GPU parity requires a WGPU adapter");
        }
        eprintln!("WGPU adapter unavailable; optional ADA-A1B device parity skipped");
        return None;
    };
    let (device, queue) = pollster::block_on(adapter.request_device(
        &wgpu::DeviceDescriptor {
            label: Some("flat-ada-a1b-parity"),
            required_features: wgpu::Features::empty(),
            required_limits: wgpu::Limits::downlevel_defaults(),
        },
        None,
    ))
    .unwrap_or_else(|error| panic!("ADA-A1B request_device failed: {error}"));
...
fn run_shader(
    ...
    let tensor_len = case.shape.tensor_len().unwrap();
    let lse_len = case.shape.lse_len().unwrap();
    ...
    let batch_heads = case.shape.batch.checked_mul(case.shape.heads).unwrap();
    let scale = case.config.resolved_scale(case.shape.head_dim).unwrap();
    let params = [
        u32::try_from(case.shape.seq_len).unwrap(),
        u32::try_from(case.shape.head_dim).unwrap(),
        u32::try_from(batch_heads).unwrap(),
        u32::from(case.config.causal),
        scale.to_bits(),
        0,
        0,
        0,
    ...
    let params_buffer = harness.device.create_buffer(&wgpu::BufferDescriptor {
        label: Some("flat-ada-a1b-params"),
    ...
    let bind_group = harness
        ...
        .create_bind_group(&wgpu::BindGroupDescriptor {
            ...
            entries: &[
                wgpu::BindGroupEntry {
                    binding: 0,
                    resource: inputs.q.as_entire_binding(),
                },
                wgpu::BindGroupEntry {
                    binding: 1,
                    resource: inputs.k.as_entire_binding(),
                },
                wgpu::BindGroupEntry {
                    binding: 2,
                    resource: inputs.v.as_entire_binding(),
                },
                wgpu::BindGroupEntry {
                    binding: 3,
        ...
        .create_command_encoder(&wgpu::CommandEncoderDescriptor { label: Some(label) });
    {
        ...
        let dispatch_x = case.shape.seq_len.div_ceil(WGSL_QUERY_ROWS);
...
#[test]
fn ada_a1b_matches_cpu_oracle_and_qualified_q4_gpu() {
    ...
    for case in cases {
        ...
        let q_gpu = input_buffer(&harness.device, &harness.queue, &q, "flat-ada-a1b-q");
        let k_gpu = input_buffer(&harness.device, &harness.queue, &k, "flat-ada-a1b-k");
        let v_gpu = input_buffer(&harness.device, &harness.queue, &v, "flat-ada-a1b-v");
