//! MAA-13d mass-aware adaptive routing frontier evidence.
//! Preregistration: docs/research/MAA_13D_MASS_AWARE_FRONTIER_PREREGISTRATION.md.
...
const PROTOCOL: &str = "maa-mass-aware-frontier/v1";
const DATASET_SEED: u64 = 0x004d_4141_2d31_3364;
...
const NORM_ONE_THRESHOLD: f64 = 1.0;
const NORM_ONE_HALF_THRESHOLD: f64 = 1.5;
const NORM_TWO_THRESHOLD: f64 = 2.0;
const HIGH_NORM_THRESHOLD: f64 = NORM_ONE_HALF_THRESHOLD;
...
fn run() -> Result<Vec<String>> {
    require(
        PROTOCOL == "maa-mass-aware-frontier/v1",
    ...
    let mut strict_aggregate = Aggregate::default();
    let mut balanced_aggregate = Aggregate::default();
    let mut recall_aggregate = Aggregate::default();
    ...
    for (case_id, case) in cases.iter().enumerate() {
        ...
        let mut tiered_strict = Vec::new();
        let mut tiered_balanced = Vec::new();
        let mut tiered_recall = Vec::new();
        ...
        for (candidate_id, candidate) in case.candidates.iter().enumerate() {
            ...
            let inner_value = inner_rule.admits(&q_signature, &key_signature)?;
            let medium_value = medium_rule.admits(&q_signature, &key_signature)?;
            let key_norm = l2_norm(&candidate.k)?;
            let high_norm = key_norm >= HIGH_NORM_THRESHOLD;
            ...
            let strict_value = near_value
                || (inner_value && key_norm >= NORM_ONE_HALF_THRESHOLD)
            ...
            if balanced_value {
                tiered_balanced.push(candidate_id);
            }
            if recall_value {
                tiered_recall.push(candidate_id);
            }
        }
        ...
        require(
            tiered_strict.iter().all(|id| tiered_balanced.contains(id))
                && tiered_balanced.iter().all(|id| tiered_recall.contains(id))
                && tiered_recall.iter().all(|id| medium.contains(id)),
            "MAA-13d tier nesting drifted",
        )?;
        ...
        let (strict_metrics, _strict_mass) =
            case_metrics(case, &tiered_strict, &top2, &dense, &score_values)?;
        let (balanced_metrics, _balanced_mass) =
            case_metrics(case, &tiered_balanced, &top2, &dense, &score_values)?;
        let (recall_metrics, _recall_mass) =
            case_metrics(case, &tiered_recall, &top2, &dense, &score_values)?;
        ...
        strict_aggregate.record(strict_metrics);
        balanced_aggregate.record(balanced_metrics);
        recall_aggregate.record(recall_metrics);
    ...
    let aggregates = [
        ...
        ("norm_aware_maa_13c", maa_aggregate.finalized()?),
        ("tiered_strict", strict_aggregate.finalized()?),
        ("tiered_balanced", balanced_aggregate.finalized()?),
        ("tiered_recall", recall_aggregate.finalized()?),
