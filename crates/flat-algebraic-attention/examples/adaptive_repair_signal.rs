//! MAA-14b structural adaptive-repair signal experiment.
//! Preregistration: docs/research/MAA_ADAPTIVE_REPAIR_SIGNAL_PREREGISTRATION.md.
...
const PROTOCOL: &str = "maa-adaptive-repair-signal/v1";
const DATASET_SEED: u64 = 0x004d_4141_2d31_3462;
const RANDOM_CONTROL_SEED: u64 = 0x4d41_4131_3462_524e;
const PROJECTOR_SEED: u64 = 0x514b_5349_474e_3133;
const CASES: usize = 640;
...
fn run() -> Result<Vec<String>> {
    require(
        ...
        "MAA-14b protocol identity drifted",
    )?;
    ...
    let trigger_arms = [
        ...
        StructuralRepairTrigger::RepairAnyGap,
        StructuralRepairTrigger::CoverageBelowSevenEighths,
        StructuralRepairTrigger::CoverageBelowThreeQuarters,
        StructuralRepairTrigger::CoverageBelowTwoThirds,
        StructuralRepairTrigger::AlwaysRepair,
    ];
    let adaptive_triggers = [
        StructuralRepairTrigger::RepairAnyGap,
        StructuralRepairTrigger::CoverageBelowSevenEighths,
        StructuralRepairTrigger::CoverageBelowThreeQuarters,
        StructuralRepairTrigger::CoverageBelowTwoThirds,
    ...
    let mut trigger_aggregates = [(); 6].map(|()| Aggregate::default());
    let mut random_aggregates = [(); 4].map(|()| Aggregate::default());
    ...
    for (trigger, aggregate) in adaptive_triggers
        .into_iter()
        .zip(random_aggregates.into_iter())
    {
        rows.push(format_row(
            &format!("matched_random_{}", trigger.label()),
            aggregate.finalize()?,
        ));
    }
    rows.push(format_row(
        "oracle_repair_if_base_mass_below_0_90",
        oracle_aggregate.finalize()?,
    ));
    Ok(rows)
}
...
mod tests {
    ...
    #[test]
    fn structural_trigger_nesting_matches_frozen_ratios() {
        for medium in 0..=CANDIDATES {
            for base in 0..=medium {
                let seven = StructuralRepairTrigger::CoverageBelowSevenEighths
                    .decide(base, medium)
                    .unwrap()
                    .repair;
                let three = StructuralRepairTrigger::CoverageBelowThreeQuarters
                    .decide(base, medium)
                    .unwrap()
                    .repair;
                let two = StructuralRepairTrigger::CoverageBelowTwoThirds
                    .decide(base, medium)
                    .unwrap()
                    .repair;
                assert!(!three || seven);
