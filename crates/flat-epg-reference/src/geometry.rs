...
///
/// Mathematical representation identity and execution position origin are
/// intentionally separate: moving a query does not create a new capability.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct EpgEmbeddingConfig {
    /// Runtime-neutral mathematical representation.
...
impl EpgEmbeddingConfig {
    ...
    /// Generation-1 compatibility constructor.
    pub fn v1(
        theta: f32,
        position_offset: usize,
        so4_dims: usize,
        ...
        if so4_dims == 0 {
            Self::so2(theta, position_offset)
        } else {
            Self::hybrid_so4(theta, position_offset, so4_dims, so4_geometry)
        }
    }
    ...
    /// SO(4) control family, or `None` for pure SO(2).
    ...
    /// Resolve a local token index to an absolute host position.
    pub fn resolve_position(self, local_index: usize) -> Result<usize, EpgError> {
        ...
        usize::try_from(self.position.resolve(local_index)?).map_err(|_| EpgError::PositionOverflow)
    }

    /// Validate this execution against one attention head and sequence.
...
/// Failures in the EPG contract, oracle adapter, or wrapped FLAT contract.
#[derive(Debug, Clone, PartialEq)]
pub enum EpgError {
    /// Runtime-neutral EPG contract error.
    Contract(EpgContractError),
    /// A host dimension does not fit the portable EPG index space.
    DimensionOverflow,
