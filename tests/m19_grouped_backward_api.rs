...
fn harness() -> Option<Harness> {
    ...
    let Ok(adapter) = adapter else {
        if std::env::var_os("FLAT_REQUIRE_WGPU").is_some() {
            panic!("M19 grouped backward API requires a WGPU adapter in the mandatory device gate");
        }
        eprintln!("WGPU adapter unavailable; optional M19 grouped backward API test skipped");
        return None;
    };
    let (device, queue) = pollster::block_on(adapter.request_device(&wgpu::DeviceDescriptor {
        label: Some("flat-m19-grouped-backward-api-test"),
...
fn fixture(len: usize, phase: f32) -> Vec<f32> {
    (0..len)
        .map(|index| {
            let x = index as f32 * 0.067 + phase;
            x.sin() * 0.75 + (x * 0.31).cos() * 0.25
        })
        .collect()
}
...
fn read_f32(
    ...
    let staging = device.create_buffer(&wgpu::BufferDescriptor {
        label: Some("flat-m19-grouped-backward-api-readback"),
        size: bytes,
...
#[test]
fn grouped_layout_preserves_native_kv_cardinality() {
...
#[test]
fn grouped_backward_public_api_matches_oracle() {
    ...
    let packed_gpu = initialized_buffer(
        ...
        "flat-m19-grouped-backward-api-forward",
    );
    ...
    let mut encoder = harness
        ...
        .create_command_encoder(&wgpu::CommandEncoderDescriptor {
            label: Some("flat-m19-grouped-backward-api"),
        });
    let layout = pipeline
        .encode(
            &harness.device,
    ...
    assert_close("dQ", &actual[..dq_end], &expected.dq);
    assert_close("dK", &actual[dq_end..dk_end], &expected.dk);
    assert_close("dV", &actual[dk_end..], &expected.dv);
}
