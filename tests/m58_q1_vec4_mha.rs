...
fn harness() -> Option<Harness> {
    ...
    let Some(adapter) = adapter else {
        if std::env::var_os("FLAT_REQUIRE_WGPU").is_some() {
            panic!("M58 Q1 vec4 MHA qualification requires a WGPU adapter");
        }
        eprintln!("WGPU adapter unavailable; optional M58 vec4 test skipped");
        return None;
    };
    let (device, queue) = pollster::block_on(adapter.request_device(
        &wgpu::DeviceDescriptor {
            label: Some("flat-m58-q1-vec4-mha"),
            required_features: wgpu::Features::empty(),
            required_limits: wgpu::Limits::downlevel_defaults(),
        },
        None,
    ))
    .unwrap_or_else(|error| panic!("M58 request_device failed: {error}"));
...
fn run_case(harness: &Harness, head_dim: usize, causal: bool) {
    ...
    let q_gpu = storage(&harness.device, &harness.queue, &q, "flat-m58-q");
    let k_gpu = storage(&harness.device, &harness.queue, &k, "flat-m58-k");
    let v_gpu = storage(&harness.device, &harness.queue, &v, "flat-m58-v");

    let pipeline = WgpuGroupedForwardPipeline::with_q1_vec4_mha(&harness.device, true).unwrap();
    assert!(pipeline.q1_vec4_enabled());
    assert_eq!(
    ...
    let prepared = pipeline
        ...
        .unwrap();
    assert_eq!(format!("{:?}", prepared.kernel_variant()), "Q1Vec4Mha");
...
#[test]
fn q1_vec4_mha_matches_oracle_for_d64_d128() {
...
#[test]
fn q1_vec4_never_claims_gqa_or_unqualified_dimensions() {
    let Some(harness) = harness() else {
        return;
    };
    let pipeline = WgpuGroupedForwardPipeline::with_q1_vec4_mha(&harness.device, true).unwrap();
