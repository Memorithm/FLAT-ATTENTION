// M12: padded variable-length sequence-major projection layout + fused RoPE + GQA/MQA.
//
// Physical storage stays dense/padded:
//   Q   [batch, padded_q_len,  q_heads  * head_dim]
//   K/V [batch, padded_kv_len, kv_heads * head_dim]
//   O   [batch, padded_q_len,  q_heads  * head_dim]
//
// Each batch element carries four u32 metadata values in params.metadata:
//   (active_q_len, active_kv_len, causal_query_offset, q_rope_offset).
// Padded K/V rows are never staged. Padded query rows produce O=0 and LSE=-inf.
// K/V RoPE keeps one global origin; Q RoPE origin is per batch element.

const WORKGROUP_SIZE: u32 = 64u;
const QUERY_ROWS: u32 = 4u;
const KV_TILE: u32 = 8u;
const MAX_HEAD_DIM: u32 = 128u;
const MAX_BATCH: u32 = 256u;
...
fn flat_attention_forward(
    ...
    if (query_start >= params.padded_q_len || q_batch_head >= q_batch_heads || params.batch > MAX_BATCH || params.head_dim == 0u || params.head_dim > MAX_HEAD_DIM || (params.head_dim & 1u) != 0u || params.q_heads == 0u || params.kv_heads == 0u || params.padded_kv_len == 0u) {
        return;
    }
    ...
    // Entirely padded query tiles can terminate immediately after materialising
    // the deterministic padded-query contract.
    if (query_start >= active_q_len) {
        var padded_row = 0u;
        loop {
            if (padded_row >= QUERY_ROWS) {
                break;
            }
            let query_pos = query_start + padded_row;
            if (query_pos < params.padded_q_len) {
                let row = batch_index * params.padded_q_len + query_pos;
                let output_base = row * q_width + q_head * params.head_dim;
                if (d0 < params.head_dim) {
                    out_and_lse[output_base + d0] = 0.0;
                }
                if (d1 < params.head_dim) {
                    out_and_lse[output_base + d1] = 0.0;
                }
                if (lane == 0u) {
                    let lse_index = output_elems + q_batch_head * params.padded_q_len + query_pos;
                    out_and_lse[lse_index] = bitcast<f32>(NEG_INF_BITS);
                }
            }
            padded_row += 1u;
        }
    ...
    let query_limit = min(active_q_len, query_start + QUERY_ROWS);
    var kv_limit = active_kv_len;
    if (params.causal != 0u) {
        kv_limit = min(active_kv_len, causal_query_offset + query_limit);
    }
    ...
    loop {
        ...
        loop {
            ...
            let row = batch_index * params.padded_kv_len + key_pos;
        ...
        loop {
            ...
            loop {
                ...
                let query_pos = query_start + qr;
                let valid_query = query_pos < active_q_len;
                let causal_query_pos = causal_query_offset + query_pos;
    ...
    loop {
        ...
        if (query_pos < params.padded_q_len) {
            let row = batch_index * params.padded_q_len + query_pos;
            let output_base = row * q_width + q_head * params.head_dim;
            let active_query = query_pos < active_q_len;
            if (active_query) {
