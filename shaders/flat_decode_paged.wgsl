...
//   page_table [mapped_pages] -> physical page index
...
@group(0) @binding(4) var<storage, read_write> out_and_lse: array<f32>;
...
fn flat_attention_decode_paged(
    ...
    let lane = local_id.x;
    if (q_head >= params.q_heads || params.kv_len == 0u || params.page_size == 0u || params.mapped_pages == 0u || params.head_dim == 0u || params.head_dim > MAX_HEAD_DIM || (params.head_dim & 1u) != 0u || params.q_heads == 0u || params.kv_heads == 0u) {
        return;
    }
    ...
    loop {
        ...
        let base = 2u * pair;
        let rotated = rope_pair(q_shared[base], q_shared[base + 1u], pair, params.q_rope_position, params.head_dim, theta);
    ...
    loop {
        ...
        loop {
            ...
            var partial = 0.0;
            if (d0 < params.head_dim) { partial += q_shared[d0] * k_shared[shared_row + d0]; }
            if (d1 < params.head_dim) { partial += q_shared[d1] * k_shared[shared_row + d1]; }
            reduce_shared[lane] = partial;
            workgroupBarrier();

            var reduction_offset = 32u;
            loop {
                if (reduction_offset == 0u) { break; }
                if (lane < reduction_offset) { reduce_shared[lane] += reduce_shared[lane + reduction_offset]; }
                workgroupBarrier();
                reduction_offset /= 2u;
            }
            ...
            if (d0 < params.head_dim) { acc0 = acc0 * alpha_shared + p_shared * v_shared[shared_row + d0]; }
            if (d1 < params.head_dim) { acc1 = acc1 * alpha_shared + p_shared * v_shared[shared_row + d1]; }
            workgroupBarrier();
            tile_row += 1u;
        }
    ...
    if (d0 < params.head_dim) { out_and_lse[q_base + d0] = acc0 * inv_sum; }
    if (d1 < params.head_dim) { out_and_lse[q_base + d1] = acc1 * inv_sum; }
    if (lane == 0u) { out_and_lse[q_width + q_head] = running_max_shared + log(running_sum_shared); }
}
