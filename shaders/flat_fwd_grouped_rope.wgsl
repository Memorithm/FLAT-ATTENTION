// FLAT-R1: native GQA/MQA + head-local RoPE + online softmax in one dispatch.
//
// Q/K arrive unrotated. Their interleaved RoPE pairs are rotated only after
// staging in workgroup memory, so no global-memory RoPE output is materialized.
// V is never rotated. The frequency schedule matches SciRust:
//   freq_j = theta^(-2*j/head_dim)
//   pos    = token_position + position_offset
//
// Dispatch:
...
fn flat_attention_forward(
    ...
    let kv_head_base = (batch_index * params.kv_heads + kv_head) * head_stride;
    ...
    // Stage raw Q for up to four rows.
    var qr = 0u;
    loop {
        if (qr >= QUERY_ROWS) {
            break;
    ...
    // Rotate Q in-place in workgroup memory. One logical item is one
    // (query-row, interleaved-pair), distributed over all 64 lanes.
    ...
    loop {
        ...
        loop {
            ...
            let global_index = kv_head_base + (tile_start + tile_row) * params.head_dim + dim;
        ...
        // Rotate the staged K tile in-place. No global-memory K-RoPE tensor.
