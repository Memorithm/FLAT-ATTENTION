// FLAT-ATTENTION M60 direct-load Q1 vec4 MHA research candidate.
//
// M58 removed Q4 query-row multiplexing, but still staged K/V through
// workgroup memory. In Q1 each K/V vec4 is consumed exactly once by the
// workgroup, so M60 removes only that non-reused staging step. The 64-lane
// reduction and online-softmax synchronization remain intentionally unchanged
// to isolate this single mechanism.
