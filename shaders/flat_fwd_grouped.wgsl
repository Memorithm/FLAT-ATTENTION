// FLAT-ATTENTION portable fused forward kernel — native GQA/MQA Q4 generation.
//
// Q is indexed by query head while K/V are indexed by grouped KV head. The
// kernel never expands or duplicates K/V. For every query head qh:
//
//   group_size = q_heads / kv_heads
//   kv_head    = qh / group_size
//
// MHA is q_heads == kv_heads. MQA is kv_heads == 1.
//
// Dispatch contract:
...
fn flat_attention_forward(
    ...
    if (query_start >= params.seq_len || q_batch_head >= q_batch_heads || params.head_dim == 0u || params.head_dim > MAX_HEAD_DIM || params.q_heads == 0u || params.kv_heads == 0u) {
        return;
    }
