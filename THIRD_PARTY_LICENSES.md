...
| Dependency | FLAT role | Current root declaration | License verification status | Runtime requirement |
|---|---|---:|---|---|
| `wgpu` | Optional portable GPU API/backend abstraction | `30.0` | Verify the exact resolved package metadata/notices from the applicable lockfile before redistribution | Only with feature `wgpu` |
| `pollster` | Optional synchronous completion of WGPU futures | `1.0` | Verify the exact resolved package metadata/notices from the applicable lockfile before redistribution | Only with feature `wgpu` |
| `ordered-float` | MSRV-compatible transitive-range constraint used by the WGPU dependency graph | `=5.4.0` | Verify the exact resolved package metadata/notices from the applicable lockfile before redistribution | Direct root dependency; pinned for Rust 1.89 compatibility |
| `naga` | WGSL parser/validator for root-package development/tests | `30.0` | Verify the exact resolved package metadata/notices from the applicable lockfile before redistribution | Root development/test only |
